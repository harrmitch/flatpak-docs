...
If you are not using one of these toolkits, you can access
...
to ensure compatibility, it is recommended to use status icons
in a supplementary manner, rather than relying on them as the only
mechanism for providing status information or access to particular
...
other softwares and exploitation of bugs in the host service, such as image decoders.

To use StatusNotifier, at a minimum, you must have the ``--talk-name=org.kde.StatusNotifierWatcher``
...
However, if needed, the ``--own-name=org.kde.*`` permission will allow this. This introduces
...
controls appear on the same side of the window across all desktops. On Linux,
...
Starting from the 5.15-22.08+ and 6.5+ branches of the ``org.kde.Platform``
runtime on Wayland, ``org.kde.WaylandDecoration.QAdwaitaDecorations`` and
``org.kde.WaylandDecoration.QGnomePlatform-decoration`` need to be installed.
Please see the upstream `usage <https://github.com/FedoraQt/QAdwaitaDecorations?tab=readme-ov-file#usage>`_
...
If the theme is not available via Flatpak extensions, or if portal or
XSettings support is lacking, Gtk and Qt applications will fall back to
...
Qt settings from the host, which usually grants it filesystem access. This
is undesirable in most cases, as it weakens the sandbox and reduces
portability. Desktop environments should properly provide portal or
...
Appearance settings, such as the Freedesktop color-scheme preference,
are also exposed similarly via the respective Settings portal. The
...
``/usr/local/share/fonts`` at ``/run/host/local-fonts``,
and ``$XDG_DATA_HOME/fonts`` at ``/run/host/user-fonts``.
